...
2. 主動查閱：
   ...
   - `./webapi/{serviceName}/documents.md`、`./service/{serviceName}/documents.md` 或 `./frontend/{projectName}/documents.md`（依 kind，若存在）— 業務規範與已知限制；再讀 `*-detail.md` 或 README 補充技術細節
   - 若 **找不到 documents.md**，主動告知：「找不到 {名稱} 的文件，請確認服務名稱是否正確？」（除非使用者已說明為新服務）
   - `./webapi/{serviceName}/scenario-flows/`（若存在）— 先列目錄，挑與症狀相關的流程讀取，了解正常業務流程以定位是哪一步出錯

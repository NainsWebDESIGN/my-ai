...
| 前端 | `./frontend/_index.md` 對應站台資訊；若有 `./frontend/{projectName}/documents.md` 一併讀取業務規範 |
| 任何後端服務（有 `documents.md`） | 讀 `./webapi/{serviceName}/documents.md` 或 `./service/{serviceName}/documents.md`（依 kind）— 驗證實作是否符合業務規範（狀態機、業務限制、必填條件等） |
| PR 新增或修改 endpoint / DTO / schema | 讀 `./{kind}/{serviceName}/detail.md`（若存在）— 確認新設計是否與現有架構一致，避免重複定義或命名衝突 |
...
1. 讀 `./webapi/_index.md`，找出本服務的呼叫方或下游相依服務

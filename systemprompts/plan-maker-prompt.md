...
- 若提到特定服務 / 前端專案名稱 → 依 kind 讀 `./webapi/_index.md` 確認 OpenAPI 路徑（`{service}.json`）與 `documents.md` 是否存在
...
- 若提到 DB table → 讀 `./db/_index.md` 確認 table 是否存在、再讀 `{db}-detail.md`；**路徑寫入 §11**
